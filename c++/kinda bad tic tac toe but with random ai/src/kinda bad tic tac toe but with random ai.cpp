...
// Description :kinda bad tac toe  in C++ with some memes and random ai
//============================================================================
//10 wide per sqaure
#include <iostream>
#include <ctime>
...
bool Checkwinner(int model[3][3],int player){
			...
			(model[2][0] ==player && model[2][1] ==player  && model[2][2]==player))//3rd piller
	{
		wins = true;
		if (player==2){
			cout <<"player O wins, beaten by a random number generator, how shamefull";
		}//which player
		else{
			cout<< "player X wins  ";
		}
...
srand(time(0));
	...
	for(int turn=1;turn<10;turn++){
		...
		else
		{
			...
			}while(model[rand1][rand2]!=0); //do
			model[rand1][rand2]=2;//0=2
			BoardUpdate(model);
